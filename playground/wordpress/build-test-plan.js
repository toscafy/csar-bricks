...
const invokeEndpoint = (endpoints, operation, done) => {
  ...
  const endpoint = endpoints[operation] || endpoints[fallbackArtifact];
...
async.series([
  ...
  function(done) {
    async.eachSeries([
      'install-svn', 'install-nodejs', 'install-grunt', 'build-wordpress'
  ...
  function(done) {
    async.eachSeries([
      'install-phpunit', 'install-mysql', 'test-wordpress'
    ], function(operation, done) {
      invokeEndpoint(endpoints, operation, (err, result) => {
        results.push(result);
        done(err);
